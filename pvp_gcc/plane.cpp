#include "plane.h"
#include<iostream>
Plane::Plane(double x,double y,double r,double m,double angle){
...
//            bullet->init(x,y,b_v_x,b_v_y,angle,RADIUS_BULLET,QUALITY_BULLET);
            ...
            //把对象加到飞机的子弹列表中
            ownBullet.push_back(poolObject);
            //记录shoot时间
            lastShoot=currentFrame*TIME_FRAME;

//            emit bullet->angleChanged(angle);
        }
...
void Plane::updateBullet(ObjectPool* bulletObjectPool)
{
    ...
    PooledObject* po;
    for(it=ownBullet.begin();it!=ownBullet.end();it++){
    	po = *it;
        FlyObject*fo=(FlyObject*)po->obj;
        double x=fo->x+fo->v_x*t;
        double y=fo->y+fo->v_y*t;
        if(x>0&&x<WIDTH_WIDGET&&y>0&&y<HEIGHT_WIDGET){
            fo->x=x;
            fo->y=y;
//            emit fo->postionChanged(x ,y);
        }else{
            //还给对象池，不再使用，busy=false
            bulletObjectPool->returnObj(*it);
            //将子弹从list中移除，不再绘制
            ownBullet.erase(it);
        }
    }
